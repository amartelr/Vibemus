...
def cleanup_library_automatic():
    print("\n" + "━"*60)
    print("🧹 VIBEMUS AUTOMATED LIBRARY CLEANUP")
    print("━"*60)
    ...
    sheet_vids = {s.get('Video ID') for s in all_songs_in_sheet if s.get('Video ID') and s.get('Playlist') != '#'}
    ...
    # 5. Execute removal in batches
    print(f"\n4. Executing removal of {len(to_remove)} songs...")
    batch_size = 25
    ...
    print("\n" + "━"*60)
    print(f"✅ CLEANUP COMPLETED: {len(to_remove)} songs removed.")
    print("━"*60 + "\n")

if __name__ == "__main__":
    cleanup_library_automatic()
